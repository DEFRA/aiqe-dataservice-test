...
Continue`
    const getsearchPagecontent =
      await searchPage.getSearchPageContent.getText()
    ...
    for(const element of searchPageContent) {
      const styles = await styling.getStyles(element, searchPageContentProperties);
    ...
    for(const element of searchPageHintText) {
      const styles = await styling.getStyles(element, searchPageHintTextProperties);
    ...
    for(const element of EnterTownOrPostcodeLabel) {
      const styles = await styling.getStyles(element, EnterTownOrPostcodeLabelProperties);
      expect(styles['font-size']).toBe('19px');
      expect(styles['line-height']).toBe('25px');
      expect(styles['font-family']).toBe('"GDS Transport", arial, sans-serif');
      expect(styles['color']).toBe('rgb(11, 12, 12)');
    ...
    for(const element of approxSearchAreaLabel) {
      const styles = await styling.getStyles(element, approxSearchAreaLabelProperties);
      expect(styles['font-size']).toBe('19px');
      expect(styles['line-height']).toBe('25px');
      expect(styles['font-family']).toBe('"GDS Transport", arial, sans-serif');
      expect(styles['color']).toBe('rgb(11, 12, 12)');
    ...
    const mileOptionsProperties = ['align-self','margin-bottom','padding','font-size','line-height','font-family','color','font-weight','height','width']
